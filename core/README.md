...
- `report_view.py`가 데일리 리포트 화면을 담당하도록 통합되었기 때문에, `데일리 리포트` 버튼 클릭 시 바로 **데일리 리포트 전용 화면**으로 이동하도록 직관적으로 수정했습니다.
...
### 2. `views/report_view.py` UI/UX 고도화
