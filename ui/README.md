...
| 변경 유형 | 수정 |
| 변경 요약 | index.html 분석 기반 UI 개선 — (1) 폼 레이아웃 3행 구조로 변경: 이름(전체폭) → 성별+달력라디오 → 생년월일+출생시간 (2) 폼 컨테이너 글래스모피즘 카드 CSS 추가 `[data-testid="stForm"]` (3) 앱 배경 conic-gradient + linear-gradient 혼합으로 교체 |
| 인터페이스 영향 | 없음 (랜딩 페이지 전용, 다른 페이지 미영향) |

| 항목 | 내용 |
| --- | --- |
| 수정 파일 | `pages/1_일반_사주.py` |
| 변경 유형 | 수정 |
